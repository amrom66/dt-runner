...
type ModelLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Model, err error)
...
// List lists all Models in the indexer.
func (s *modelLister) List(selector labels.Selector) (ret []*v1.Model, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Model))
	})
	return ret, err
}
...
type ModelNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.Model, error)
...
// List lists all Models in the indexer for a given namespace.
func (s modelNamespaceLister) List(selector labels.Selector) (ret []*v1.Model, err error) {
...
// Get retrieves the Model from the indexer for a given namespace and name.
func (s modelNamespaceLister) Get(name string) (*v1.Model, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("model"), name)
	}
	return obj.(*v1.Model), nil
}
